...
/**
 * One day's hours. `ranges` is kept while a day is switched off, so switching it
 * back on restores the hours that were there rather than a blank row.
 */
...
export function blankWeeklyAvailability(): WeeklyAvailability {
  ...
  for (const day of DAYS_OF_WEEK) {
    weekly[day] = { enabled: false, ranges: [{ ...DEFAULT_RANGE }] };
  }
  return weekly;
}
...
export function toWeeklyAvailability(availableTime: HealthcareServiceAvailableTime[] | undefined): WeeklyAvailability {
  ...
  for (const day of DAYS_OF_WEEK) {
    ...
    if (ranges.length > 0) {
      weekly[day] = { enabled: true, ranges };
    }
  }
 ...
 * @returns True when any day is switched on with at least one block
 */
export function hasAnyAvailableDay(weekly: WeeklyAvailability): boolean {
  return DAYS_OF_WEEK.some((day) => weekly[day].enabled && weekly[day].ranges.length > 0);
}
