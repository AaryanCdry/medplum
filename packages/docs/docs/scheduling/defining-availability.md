...
Rather than hand-authoring the [`availability` extension](#availability-extension), the [`@medplum/react`](/docs/react) library provides a `ScheduleAvailabilityEditor` component for editing a Schedule's weekly `availability` override for a given visit service type. It implements the [override behavior](#override-behavior) described above through a single switch: with custom availability off, the Schedule inherits the [service-level default](#service-level-availability) and the days are shown read-only; switching it on writes an override, seeded from that default so the editor starts from the hours currently in effect. A reset link reloads the service default hours without giving up the override. It is used in the [Medplum Provider](https://github.com/medplum/medplum/tree/main/examples/medplum-provider) example app.

The component renders form content only, so you choose the container: place it inline in a page, or wrap it in a Modal or Drawer. It is controlled, returning an updated `Schedule` for you to persist (for example, via `medplum.updateResource`). `@medplum/core` exports the framework-agnostic helpers it is built on, for reading and writing the `availability` override without the UI (for example, in a bot or a custom editor).

Omitting the `schedule` prop edits the service's own [default hours](#service-level-availability) instead, returning an updated `HealthcareService` with its `availableTime` rewritten. It is the same weekly form over the same data, since the Schedule override stores exactly what `HealthcareService.availableTime` holds; only the destination differs. Editing the default changes the hours for every Schedule that has no override of its own, so the two modes are worth keeping distinct in your UI. Because the service has no default to fall back on, the inherit switch and the reset link are not shown.

The two modes also treat an empty week differently, and in opposite directions. An override with no available days cannot be written at all, since it serializes to an extension with no value and fails FHIR constraint `ext-1`, so the editor refuses to save it. A service with no `availableTime` is instead unrestricted, per [implicit availability](#the-concept-of-implicit-availability), so clearing every day makes it bookable around the clock rather than never. That is a real thing to want and the opposite of what it looks like, so the editor asks for it to be confirmed rather than blocking it, and omits the field rather than writing an empty array.

Each day holds one or more blocks of hours, picked from a list at five minute intervals. Rather than validate after the fact, the editor only offers the times still free on that day: a block cannot start before the previous one ends, and the last block of the day runs no later than midnight. Overlapping blocks and windows that run past the end of the day are therefore unreachable. A full 24 hours is expressed as a block from 12:00 AM to 12:00 AM, which is saved with the `allDay` flag.

Typing filters that list rather than entering free text, so a time can be reached by keyboard without opening the way to a value outside the bounds. Leaving the field takes whichever time the list has highlighted, the same one Enter would have submitted, so tabbing on to the next field does not discard what was typed. Typing something no available time matches leaves the field as it was.

A start time may be set past its own end time, so a later block can be opened without editing both fields in order. When that happens the end time moves to an hour after the new start, stopping at midnight. The move flashes the end time and is announced to assistive technology.

Times written elsewhere need not fall on that five minute interval. The editor shows such a time as it is rather than rounding a value nobody asked it to change, and marks the field so the difference is visible. The interval is measured from midnight, so a block stored as 9:07 AM still offers 9:10 AM and 9:15 AM alongside it. Picking any new time replaces the stored one with a value on the interval.

The editor does not author [windows that cross midnight](#windows-that-cross-midnight), but it does read them. On mount, a stored window like Friday 10:00 PM to 6:00 AM is split at the day boundary into Friday 10:00 PM to 12:00 AM plus Saturday 12:00 AM to 6:00 AM, which the scheduling operations read as the same bookable hours. Overlapping entries written by the API are likewise sorted and merged into single blocks. Availability authored elsewhere therefore still opens in the editor rather than locking it, though saving rewrites it into the split form.
