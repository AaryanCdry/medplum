...
Typing narrows that list rather than entering free text, so a time can be reached by keyboard without opening the way to a value outside the bounds. Typing a time in full, minutes included, adds it to the list, so a block can start at 3:03 PM even though the quarter hours skip it. The bounds still hold: a time outside them is not offered however it is typed. Leaving the field takes whichever time the list has highlighted, the same one Enter would have submitted, so tabbing on to the next field does not discard what was typed. Typing something no available time matches leaves the field as it was.
...
The interval decides what is listed, not what a block may be set to, so times written elsewhere need not fall on it. The editor shows such a time as it is rather than rounding a value nobody asked it to change, and lists it as the current selection. The interval itself is measured from midnight, so a block stored as 9:07 AM still offers 9:15 AM and 9:30 AM alongside it.
